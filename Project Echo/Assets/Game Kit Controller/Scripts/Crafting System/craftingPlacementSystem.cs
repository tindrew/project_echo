...
//				currentObjectToPlaceMesh.transform.position += Vector3.Cross (hit.normal, currentBounds.center);
	...
	void selectNextOrPreviousObjectForPlacement (bool state)
	{
		...
		int currentAmount = mainCraftingSystem.getInventoryObjectAmountByName (currentObjectName);

		if (currentAmount > 0) {
			GameObject currentObjectMesh = mainCraftingSystem.getInventoryMeshByName (currentObjectName);
			...
			if (showDebugPrint) {
				print ("not more units of " + currentObjectName + " available");
			}
		}
	}
}
