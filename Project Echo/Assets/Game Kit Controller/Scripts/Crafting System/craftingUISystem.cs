...
public class craftingUISystem : ingameMenuPanel
{
	...
	[TextArea (3, 5)]public string messageOnObjectInProcess = "-OBJECT- creation in process.";
	...
	public void confirmToCreate ()
	{
		...
		if (craftObjectInTimeResult) {
			bool allObjectsCanbeCreated = canObjectBeCreated (currentObjectCategorySelectedName, currentObjectSelectedName, currentAmountToCreate, true);

			print (allObjectsCanbeCreated);

			if (allObjectsCanbeCreated) {
				craftObjectInTime (timeToCraftObject);

				string newMessage = messageOnObjectInProcess;
