		...
		if (storeDurabilityValuesOfPiecesEnabled) {
			...
			if (durabilityIndex >= 0) {
				...
				if (currentPiecesDurabilityList [durabilityIndex] <= 0) {
					...
					if (useInventoryManager) {
						mainInventoryManager.addOrRemoveDurabilityAmountToObjectByName (objectName, -1, currentPiecesDurabilityList [durabilityIndex], true);
					} else {
		...
		if (storeDurabilityValuesOfPiecesEnabled) {
			for (int i = 0; i < currentPiecesList.Count; i++) {
				if (currentPiecesDurabilityList [i] != -1) {
					...
					if (useInventoryManager) {
						mainInventoryManager.addOrRemoveDurabilityAmountToObjectByName (currentPiecesList [i], -1, currentPiecesDurabilityList [i], true);
					} else {
