				...
				for (int i = 0; i < pathManager.wayPoints.Count; i++) {
					...
					if (currentWaypoint) {
						...
						if (pathManager.useHandleForVertex) {
							...
							var fmh_172_53_638404008000361890 = Quaternion.identity; newPoint = Handles.FreeMoveHandle (oldPoint, pathManager.handleRadius, new Vector3 (.25f, .25f, .25f), Handles.CircleHandleCap);
