					...
					for (int j = 0; j < buildingList [i].buildingFloorsList.Count; j++) {
						if (manager.buildingList [i].buildingFloorsList [j].floor != null) {
							...
							if (manager.useHandleForVertex) {
								...
								var fmh_136_62_638404008002092705 = Quaternion.identity; Vector3 newPoint = Handles.FreeMoveHandle (oldPoint, manager.handleRadius, new Vector3 (.25f, .25f, .25f), Handles.CircleHandleCap);
