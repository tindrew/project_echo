		...
		if (patrol.showGizmo) {
			...
			for (int i = 0; i < patrol.patrolList.Count; i++) {
				...
				for (int j = 0; j < patrol.patrolList [i].wayPoints.Count; j++) {
					if (patrol.patrolList [i].wayPoints [j] != null) {
						...
						if (patrol.useHandleForVertex) {
							...
							var fmh_76_61_638404007999410272 = Quaternion.identity; Vector3 newPoint = Handles.FreeMoveHandle (oldPoint, patrol.handleRadius, new Vector3 (.25f, .25f, .25f), Handles.CircleHandleCap);
				...
				if (patrol.useHandleForVertex) {
					...
					var fmh_97_59_638404007999413270 = Quaternion.identity; Vector3 newPoint = Handles.FreeMoveHandle (oldPoint, patrol.handleRadius, new Vector3 (.25f, .25f, .25f), Handles.CircleHandleCap);
