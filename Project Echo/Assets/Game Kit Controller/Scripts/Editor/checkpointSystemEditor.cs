			...
			for (int i = 0; i < manager.checkPointList.Count; i++) {
				if (manager.checkPointList [i] != null) {
					...
					if (manager.useHandleForVertex) {
						...
						var fmh_71_60_638404007999360248 = Quaternion.identity; Vector3 newPoint = Handles.FreeMoveHandle (oldPoint, manager.handleRadius, new Vector3 (.25f, .25f, .25f), Handles.CircleHandleCap);
