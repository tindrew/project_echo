						...
						for (int i = 0; i < builder.eventTriggerList.Count; i++) {
							if (builder.eventTriggerList [i]) {
								...
								if (builder.useHandleForVertex) {
									...
									var fmh_117_55_638404007999577171 = Quaternion.identity; newPoint = Handles.FreeMoveHandle (oldPoint, builder.handleRadius, snapValue, Handles.CircleHandleCap);
					...
					for (int i = 0; i < builder.verticesPosition.Count; i++) {
						if (builder.verticesPosition [i]) {
							...
							if (builder.useHandleForVertex) {
								...
								var fmh_168_54_638404007999581581 = Quaternion.identity; newPoint = Handles.FreeMoveHandle (oldPoint, builder.handleRadius, snapValue, Handles.CircleHandleCap);
								if (EditorGUI.EndChangeCheck ()) {
