			...
			for (int i = 0; i < platform.wayPoints.Count; i++) {
				if (platform.wayPoints [i] != null) {
					...
					if (platform.useHandleForVertex) {
						...
						var fmh_103_52_638404007999888142 = Quaternion.identity; newPoint = Handles.FreeMoveHandle (oldPoint, platform.handleRadius, new Vector3 (.25f, .25f, .25f), Handles.CircleHandleCap);
