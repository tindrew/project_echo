...
public class playerCameraEditor : Editor
{
	...
	SerializedProperty defaultNearClipPlane;
	...
	SerializedProperty fullBodyAwarenessEnabled;

	SerializedProperty fullBodyAwarenessActive;

	SerializedProperty defaultFullBodyAwarenessStateName;

	SerializedProperty defaultFullBodyAwarenessCrouchStateName;

	SerializedProperty defaultFullBodyAwarenessAimStateName;

	SerializedProperty defaultFullBodyAwarenessCrouchAimStateName;

	SerializedProperty defaultFullBodyAwarenessDualAimStateName;

	SerializedProperty defaultFullBodyAwarenessCrouchDualAimStateName;
	...
	SerializedProperty customOriginalLockedCameraPivotPosition;
	SerializedProperty useCustomOriginalLockedCameraPivotPositionTransform;
	SerializedProperty customOriginalLockedCameraPivotPositionTransform;
	...
	SerializedProperty eventOnEnableFBAInEditor;
	SerializedProperty eventOnDisableFBAInEditor;

	SerializedProperty useHeadColliderOnFBA;
	SerializedProperty headColliderOnFBA;
	SerializedProperty useHeadColliderOnThirdPerson;

	SerializedProperty eventToActivateHeadColliderOnFBA;
	SerializedProperty eventToDeactivateHeadColliderOnFBA;
	SerializedProperty eventToActivateHeadColliderOnFBAOnEditor;
	SerializedProperty eventToDeactivateHeadColliderOnFBAOnEditor;
	...
	void OnEnable ()
	{
		...
		defaultNearClipPlane = serializedObject.FindProperty ("defaultNearClipPlane");
		...
		fullBodyAwarenessEnabled = serializedObject.FindProperty ("fullBodyAwarenessEnabled");

		fullBodyAwarenessActive = serializedObject.FindProperty ("fullBodyAwarenessActive");

		defaultFullBodyAwarenessStateName = serializedObject.FindProperty ("defaultFullBodyAwarenessStateName");

		defaultFullBodyAwarenessCrouchStateName = serializedObject.FindProperty ("defaultFullBodyAwarenessCrouchStateName");

		defaultFullBodyAwarenessAimStateName = serializedObject.FindProperty ("defaultFullBodyAwarenessAimStateName");

		defaultFullBodyAwarenessCrouchAimStateName = serializedObject.FindProperty ("defaultFullBodyAwarenessCrouchAimStateName");

		defaultFullBodyAwarenessDualAimStateName = serializedObject.FindProperty ("defaultFullBodyAwarenessDualAimStateName");

		defaultFullBodyAwarenessCrouchDualAimStateName = serializedObject.FindProperty ("defaultFullBodyAwarenessCrouchDualAimStateName");
		...
		customOriginalLockedCameraPivotPosition = serializedObject.FindProperty ("customOriginalLockedCameraPivotPosition");
		useCustomOriginalLockedCameraPivotPositionTransform = serializedObject.FindProperty ("useCustomOriginalLockedCameraPivotPositionTransform");
		customOriginalLockedCameraPivotPositionTransform = serializedObject.FindProperty ("customOriginalLockedCameraPivotPositionTransform");
		...
		eventOnEnableFBAInEditor = serializedObject.FindProperty ("eventOnEnableFBAInEditor");
		eventOnDisableFBAInEditor = serializedObject.FindProperty ("eventOnDisableFBAInEditor");

		useHeadColliderOnFBA = serializedObject.FindProperty ("useHeadColliderOnFBA");
		headColliderOnFBA = serializedObject.FindProperty ("headColliderOnFBA");
		useHeadColliderOnThirdPerson = serializedObject.FindProperty ("useHeadColliderOnThirdPerson");

		eventToActivateHeadColliderOnFBA = serializedObject.FindProperty ("eventToActivateHeadColliderOnFBA");

		eventToDeactivateHeadColliderOnFBA = serializedObject.FindProperty ("eventToDeactivateHeadColliderOnFBA");
	...
	public override void OnInspectorGUI ()
	{
		...
		EditorGUILayout.PropertyField (fullBodyAwarenessEnabled);

		GUILayout.EndVertical ();

		EditorGUILayout.Space ();

		GUILayout.BeginVertical ("Change View Settings", "window");
		...
		EditorGUILayout.Space ();
		EditorGUILayout.PropertyField (defaultFullBodyAwarenessStateName);
		EditorGUILayout.PropertyField (defaultFullBodyAwarenessCrouchStateName);
		EditorGUILayout.PropertyField (defaultFullBodyAwarenessAimStateName);
		EditorGUILayout.PropertyField (defaultFullBodyAwarenessCrouchAimStateName);
		EditorGUILayout.PropertyField (defaultFullBodyAwarenessDualAimStateName);
		EditorGUILayout.PropertyField (defaultFullBodyAwarenessCrouchDualAimStateName);
		EditorGUILayout.Space (); 
		...
		if (usedByAI.boolValue) {
			...
			GUILayout.BeginVertical ("2.5d Locked Settings On AI", "window");
			EditorGUILayout.PropertyField (customOriginalLockedCameraPivotPosition);
			EditorGUILayout.PropertyField (useCustomOriginalLockedCameraPivotPositionTransform);
			if (useCustomOriginalLockedCameraPivotPositionTransform.boolValue) {
				EditorGUILayout.PropertyField (customOriginalLockedCameraPivotPositionTransform);
			}
			GUILayout.EndVertical ();
		}
		GUILayout.EndVertical ();
		...
		if (GUILayout.Button ("Event Settings")) {
		...
		if (showEventSettings.boolValue) {
			...
			GUI.color = Color.cyan;
			EditorGUILayout.HelpBox ("Event Settings", MessageType.None);
			GUI.color = Color.white;
			...
			GUILayout.BeginVertical ("Event On FBA Person View Change Settings", "window");
			...
			if (useEventOnFBAViewChange.boolValue) {
				...
				EditorGUILayout.PropertyField (eventOnEnableFBAInEditor);
				EditorGUILayout.PropertyField (eventOnDisableFBAInEditor);
			}
			GUILayout.EndVertical ();

			EditorGUILayout.Space ();

			GUILayout.BeginVertical ("Other FBA Settings", "window");
			EditorGUILayout.PropertyField (useHeadColliderOnFBA);
			EditorGUILayout.PropertyField (useHeadColliderOnThirdPerson);
			if (useHeadColliderOnFBA.boolValue || useHeadColliderOnThirdPerson.boolValue) {
				EditorGUILayout.PropertyField (headColliderOnFBA);

				EditorGUILayout.Space ();

				EditorGUILayout.PropertyField (eventToActivateHeadColliderOnFBA);
				EditorGUILayout.PropertyField (eventToDeactivateHeadColliderOnFBA);

				EditorGUILayout.Space ();

				EditorGUILayout.PropertyField (eventToActivateHeadColliderOnFBAOnEditor);
				EditorGUILayout.PropertyField (eventToDeactivateHeadColliderOnFBAOnEditor);
			}
			GUILayout.EndVertical ();
		...
		if (!Application.isPlaying) {
			...
			if (!checkCamera) {
				if (camera.isFirstPersonActive ()) {
					currentCamera = "FIRST PERSON";
				} else {
					if (camera.isFullBodyAwarenessActive ()) {
						currentCamera = "FIRST PERSON FBA";
					} else {
	...
	void showCameraStateElementInfo (SerializedProperty list, int stateIndex)
	{
		...
		EditorGUILayout.PropertyField (list.FindPropertyRelative ("followTransformPosition"));
		if (list.FindPropertyRelative ("followTransformPosition").boolValue) {
			EditorGUILayout.PropertyField (list.FindPropertyRelative ("transformToFollow"));

			EditorGUILayout.Space ();

			EditorGUILayout.PropertyField (list.FindPropertyRelative ("useFollowTransformPositionOffset"));
			if (list.FindPropertyRelative ("useFollowTransformPositionOffset").boolValue) {
				EditorGUILayout.PropertyField (list.FindPropertyRelative ("followTransformPositionOffset"));
			}
		}
