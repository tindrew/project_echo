			...
			for (int i = 0; i < manager.waypointList.Count; i++) {
				if (manager.waypointList [i].waypointTransform != null) {
					...
					if (manager.useHandleForWaypoints) {
						...
						var fmh_89_52_638404007999419659 = Quaternion.identity; newPoint = Handles.FreeMoveHandle (oldPoint, manager.handleRadius, new Vector3 (.25f, .25f, .25f), Handles.CircleHandleCap);
