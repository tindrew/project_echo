...
public class playerWeaponsManager : MonoBehaviour
{
	...
	bool forceWeaponToUseQuickDrawKeepWeaponActive;
		...
		if (state) {
			...
			if (carryingWeaponInThirdPerson) {
				...
				if (setIKHandsStateResult) {
					if (usingDualWeapon) {
						currentRightIKWeapon.enableOrDisableIKOnHands (true);
						currentLeftIkWeapon.enableOrDisableIKOnHands (true);
					} else {
						if (currentIKWeapon != null) {
							currentIKWeapon.enableOrDisableIKOnHands (true);
						}
					}
				}
			}
				...
				if (isPlayerCrouching () || deactivateIKIfNotAiming || usingDualWeapon) {
					...
					if (usingDualWeapon) {
						if (currentRightIKWeapon.thirdPersonWeaponInfo.rightHandDualWeaopnInfo.deactivateIKIfNotAiming || isPlayerCrouching ()) {
							currentRightIKWeapon.enableOrDisableIKOnHands (false);
						}

						if (currentLeftIkWeapon.thirdPersonWeaponInfo.leftHandDualWeaponInfo.deactivateIKIfNotAiming || isPlayerCrouching ()) {
							currentLeftIkWeapon.enableOrDisableIKOnHands (false);
						}
					} else {
						if (currentIKWeapon != null) {
							currentIKWeapon.enableOrDisableIKOnHands (false);
						}
					}
...
//			print (weaponsList [i].isWeaponEnabled ());
	...
	public void enableOrDisableWeaponCursor (bool value)
	{
		...
		if (weaponCursorRegular != null) {
			...
			if (fullBodyAwarenessActive) {
				if (currentWeaponSystem.isUsingSightFullBodyAwareness ()) {
...
//								changeSingleWeapon (dualWeaponNameToSetAsSingle);
	...
	public void launchWeaponRigidbodyWhenDropWeapon (IKWeaponSystem weaponToDrop, bool dropOnlyWeaponMesh, bool ignoreRagdollCollision)
	{
		...
		Vector3 forceDirection = dropWeaponForceThirdPerson * transform.forward;

		if (holdingDropButtonToIncreaseForce) {
			forceDirection = currentDropForce * transform.forward;
		}
		...
		if (!isThirdPersonView) {
			forceDirection = dropWeaponForceFirstPerson * playerCameraManager.mainCameraTransform.forward;

			if (holdingDropButtonToIncreaseForce) {
				forceDirection = currentDropForce * playerCameraManager.mainCameraTransform.forward;
			}
		}
