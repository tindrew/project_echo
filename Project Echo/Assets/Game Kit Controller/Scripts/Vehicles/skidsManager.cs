...
public class skidsManager : MonoBehaviour
{
	...
	public int AddSkidMark (Vector3 pos, Vector3 normal, float intensity, int lastIndex)
	{
		...
		if (lastIndex != -1) {
			...
			curr.positionLeft = curr.position + (markWidth * 0.5f) * xDir;

			curr.positionRight = curr.position - (markWidth * 0.5f) * xDir;
			...
			if (last.lastIndex == -1) {
				...
				last.positionLeft = curr.position + (markWidth * 0.5f) * xDir;

				last.positionRight = curr.position - (markWidth * 0.5f) * xDir;
			}
		}
